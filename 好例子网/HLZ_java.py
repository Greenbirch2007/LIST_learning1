...
def next_page():
    for i in range(1,146):  # selenium 循环翻页成功！
...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into HLZ_java (title,type,links,stars,downloadNums) values (%s,%s,%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except :
...
if __name__ == "__main__":
    big_list = []
    url = 'https://www.haolizi.net/examples/java_1.html'
